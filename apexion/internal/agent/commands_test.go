...
func TestLoadCustomCommands(t *testing.T) {
	dir := t.TempDir()
	cmdDir := filepath.Join(dir, ".apexion", "commands")
...
func TestCommandDirs(t *testing.T) {
	...
	count := 0
	for _, d := range dirs {
		if strings.Contains(d, ".apexion/commands") {
			count++
		}
	}
	// Should have exactly one .apexion/commands entry (from cwd), not two.
	if count != 1 {
		t.Errorf("expected 1 .apexion/commands dir when cwd==gitRoot, got %d: %v", count, dirs)
	}
}
...
func TestCommandDirs_DifferentRoots(t *testing.T) {
	...
	if found < 2 {
		t.Errorf("expected at least 2 .apexion/commands dirs when cwd!=gitRoot, got %d: %v", found, dirs)
	}
}
