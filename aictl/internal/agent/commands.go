...
func commandDirs(cwd, gitRoot string) []string {
	...
	if home, err := os.UserHomeDir(); err == nil {
		dirs = append(dirs, filepath.Join(home, ".config", "aictl", "commands"))
	}

	if gitRoot != "" && gitRoot != cwd {
		dirs = append(dirs, filepath.Join(gitRoot, ".aictl", "commands"))
	}

	dirs = append(dirs, filepath.Join(cwd, ".aictl", "commands"))
...
func formatCommandList(commands map[string]*CustomCommand) string {
	if len(commands) == 0 {
		return "No custom commands found.\nPlace .md files in ~/.config/aictl/commands/ or .aictl/commands/"
	}
	...
	sb.WriteString("\nSource directories: ~/.config/aictl/commands/, .aictl/commands/")
	return sb.String()
}
