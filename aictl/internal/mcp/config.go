...
// 允许 aictl 连接外部 MCP server 并将其工具纳入 agent 的工具集。
package mcp
...
//  1. ~/.config/aictl/mcp.json （全局配置）
//  2. <cwd>/.aictl/mcp.json   （项目配置）
//
...
func LoadMCPConfig(cwd string) (*MCPConfig, error) {
	...
	if err == nil {
		globalPath := filepath.Join(home, ".config", "aictl", "mcp.json")
	...
	if cwd != "" {
		projectPath := filepath.Join(cwd, ".aictl", "mcp.json")
