...
`apexion` 是一个开源的 AI 编程助手命令行工具，使用 **Go 1.24** 编写。它可以连接多种大语言模型（Anthropic Claude、DeepSeek、OpenAI 等），在终端中提供类似 Claude Code 的交互式编程辅助体验。
...
│   ├── chat.go                    # 交互式对话模式
│   ├── run.go                     # 单次执行模式（非交互）
│   ├── init.go                    # apexion init 初始化配置
│   └── version.go                 # apexion version 版本信息
└── internal/
    ├── agent/
...
配置文件路径：`~/.config/apexion/config.yaml`
...
```bash
git clone https://github.com/apexion-ai/apexion.git
cd apexion/apexion
go build -o apexion .
sudo mv apexion /usr/local/bin/
```
...
# 单次执行（非交互，适合脚本）
apexion run "帮我找出所有超过 100 行的 Go 文件"

# 指定 Provider 和 Model
apexion --provider anthropic --model claude-opus-4-6

# 强制关闭 TUI
apexion --tui=false

# 跳过所有工具确认（谨慎使用）
apexion --auto-approve
