...
func TestLoad_EnvOverrides(t *testing.T) {
	...
	t.Setenv("AICTL_PROVIDER", "deepseek")
	...
	if cfg.Provider != "deepseek" {
		t.Errorf("AICTL_PROVIDER should override, got %q", cfg.Provider)
	}
	if cfg.Model != "custom-model" {
		t.Errorf("LLM_MODEL should override, got %q", cfg.Model)
	}
	// LLM_API_KEY applies to the provider active at config parse time (openai, before AICTL_PROVIDER override).
	// But AICTL_PROVIDER runs after LLM_API_KEY, so key is on "openai".
